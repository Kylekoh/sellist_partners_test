...
function CreateEventItem(props) {
  ...
  //02.01
  const [eventURL, setEventURL] = useState("http://");
  ...
  const [isValidEventUrl, setIsValidEventUrl] = useState(false);
  const [isValidShopUrl, setIsValidShopUrl] = useState(false);
  const [eventStartDate, setEventStartDate] = useState("");
  const [eventEndDate, setEventEndDate] = useState("");
  const [sponsored, setSponsored] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  // const eventUrlChk = isValidEventUrl.every(el => el);
  // const isValidShopUrl = isValidShopUrl.every(el => el);
  // const shopUrlChk = isValidShopUrl.every(el => el);
  console.log("샵 URL", isValidShopUrl);
  ...
  useEffect(() => {
    console.log("이벤트 URL 값", eventURL);
    const urlRegex = require("url-regex");
    const comChk = eventUrl?.includes("co.kr") || eventURL?.includes(".com");

    if (urlRegex({ exact: true }).test(eventURL) && comChk) {
      setIsValidEventUrl(true);
      dispatch(setEventUrl(eventURL));
    } else if (!comChk) {
      setIsValidEventUrl(false);
              ...
              <InputWrap>
                ...
                <div style={{ position: "relative" }}>
                  <EventUrlInput
                    ...
                    value={eventURL}
                    // defaultValue={eventURL ? eventURL : "http://"}
                    // startdate={startdate}
                    isValidEventUrl={isValidEventUrl}
                  />
                  {/* {!isValidEventUrl[0] && (
